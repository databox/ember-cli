...
class EmberApp {
  ...
  constructor(defaults, options) {
    ...
    this._filesToCleanup = this._copySharedFiles(options.sharedFiles);
  ...
  */
  _initOptions(options) {
    ...
    let appPath = this._resolveLocal(this._folderName);
    let appTree;
    ...
    if (experiments.MODULE_UNIFICATION) {
      ...
      this._stylesPath = existsSync(srcStylesPath) ? srcStylesPath : this._resolveLocal(this._folderName + '/styles');
    } else {
      this._stylesPath = this._resolveLocal(this._folderName + '/styles');
    }
    let stylesTree = new UnwatchedDir(this._stylesPath);

    let templatesPath = this._resolveLocal(this._folderName + '/templates');
  ...
  */
  ...
  /**
    Callback to cleanup sharedFiles that were copied before building

    @method cleanupFiles
    @return
  */
  cleanupFiles() {
    this._filesToCleanup.forEach(function(file){
      try {
        fs.removeSync(file);
      } catch (err) {
        logger.info(`Cannot remove \`${file}\`. ${err}`);
      }
    });
  }
  ...
  /**
    @private
    @method _copySharedFiles
  */
  _copySharedFiles(sharedFiles) {
    ...
    if (sharedFiles && sharedFiles.length) {
      ...
      let sourceRoot = 'app';
      let targetRoot = this._folderName;

      sharedFiles.forEach(function(file){
        let sourceFile = self._resolveLocal(sourceRoot + '/' + file);
        let targetFile = self._resolveLocal(targetRoot + '/' + file);
        try {
          fs.copySync(sourceFile, targetFile);
          copiedFiles.push(targetFile);
        } catch (error) {
          logger.info(`Cannot copy \`${file}\`. ${error}`);
        }
      });
    }
